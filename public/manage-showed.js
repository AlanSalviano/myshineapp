...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    function renderTable(data) {
        ...
        data.forEach((appointment, index) => {
            ...
            row.innerHTML = `
                ...
                <td class="p-4"><input type="number" value="${appointment.pets}" class="w-20 bg-transparent border border-border rounded-md px-2"></td>
                <td class="p-4"><input type="text" class="w-32 bg-transparent border border-border rounded-md px-2"></td>
                <td class="p-4"><input type="text" class="w-20 bg-transparent border border-border rounded-md px-2" placeholder="$0.00"></td>
                <td class="p-4"><input type="text" class="w-32 bg-transparent border border-border rounded-md px-2"></td>
                <td class="p-4"><input type="text" class="w-32 bg-transparent border border-border rounded-md px-2"></td>
                <td class="p-4"><input type="text" class="w-32 bg-transparent border border-border rounded-md px-2"></td>
                <td class="p-4">
    ...
    // Event listener para a ação de salvar (exemplo)
    tableBody.addEventListener('click', (event) => {
        if (event.target.classList.contains('save-btn')) {
            const row = event.target.closest('tr');
            // Lógica para coletar os dados dos inputs e salvar
            const inputs = row.querySelectorAll('input');
            const rowData = {
                date: row.cells[0].textContent,
                customers: row.cells[1].textContent,
            ...
            alert('Dados salvos! (Esta é apenas uma demonstração)');
        }
    });
